...
| `bundle-content` | Geeft een waarschuwing als een bundel initiële inhoud bevat die is opgegeven met Sling-Initial-Content, wat problematisch is in de geclusterde AEM as a Cloud Service-omgeving. De waarschuwing ziet er als volgt uit: <p> </p> `[WARNING] org.acme:mybundle:0.0.1-SNAPSHOT: Found initial content : [/]` <p> </p>Zie Documentatie opnieuw aanwijzen als u problemen wilt oplossen bij het omzetten van de eerste inhoud in instructies voor het opnieuw aanwijzen. | Ja | Ja |
...
| `api-regions`<p> </p>`api-regions-check-order`<p> </p>`api-regions-dependencies`<p> </p>`api-regions-duplicates` | Deze analysatoren controleren sommige details met betrekking tot het [ inhoudspakket aan het proces van de eigenschapmodelomzetting ](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html?lang=en#deploying) dat artefacten creeert die aan het Verschuivende Model van de Eigenschap in overeenstemming zijn. Eventuele fouten moeten worden gemeld aan de Adobe Klantenondersteuning. | Ja | Ja |
