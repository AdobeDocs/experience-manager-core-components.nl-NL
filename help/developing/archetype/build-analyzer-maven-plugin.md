...
Zie de documentatie [van de](https://github.com/adobe/aemanalyser-maven-plugin/blob/main/aemanalyser-maven-plugin/README.md) Geweven Insteekmodule voor informatie over hoe te om het in een AEM in kaart te brengen project te omvatten.
...
| `api-regions`<p> </p>`api-regions-check-order`<p> </p>`api-regions-dependencies`<p> </p>`api-regions-duplicates` | Deze analysatoren controleren sommige details met betrekking tot het [inhoudspakket om het omzettingsproces](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html?lang=en#deploying) van het model te voorzien dat artefacten tot stand brengt die aan het Model van de Eigenschap van de Verkoop aanpassen. Eventuele fouten moeten worden gemeld aan de Adobe Klantenondersteuning. | Ja | Ja |
