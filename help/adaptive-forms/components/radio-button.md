...
## Ondersteuning voor veldset en legenda voor keuzerondje(v2)
...
Aangepaste Forms Core-componenten gebruiken semantische `<fieldset>` en `<legend>` HTML-elementen om de toegankelijkheid voor groepen keuzerondjes te verbeteren. Met deze elementen kunnen schermlezers de formulierstructuur nauwkeurig interpreteren en de relaties tussen groepslabels en hun opties weergeven.

Componenten voor keuzerondjes (v2) worden binnen een `<fieldset>` -element gerenderd, waarbij het groeplabel in een `<legend>` als eerste onderliggende element wordt opgenomen. Overbodige of ongeldige ARIA-kenmerken worden verwijderd en afzonderlijke labels worden correct gekoppeld aan de respectieve invoer. Op deze manier zorgen schermlezers ervoor dat elk keuzerondje en de bijbehorende groepcontext goed worden gelezen.

![ radioknoop ](/help/adaptive-forms/assets/radiobutton-fieldset.png)
