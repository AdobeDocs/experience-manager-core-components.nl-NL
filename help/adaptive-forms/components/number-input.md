...
Deze component kan worden gebruikt om numerieke gegevens zoals leeftijd, hoeveelheid, en meer te verzamelen. en kan ook worden gebruikt om wiskundige bewerkingen uit te voeren, zoals optellen en aftrekken. Deze component kan ook worden gebruikt om de numerieke gegevens te valideren die de gebruiker heeft ingevoerd.
...
* **Gegevensbereik**: U kunt numerieke velden gebruiken om een reeks geldige waarden in te stellen met behulp van de kenmerken min, max en step.
...
* **Weergave-indeling** - Met deze optie kunt u opties selecteren uit verschillende numerieke notaties voor gehele getallen voor weergave. Wanneer de gebruiker een optie selecteert in het menu **Type** vervolgkeuzemenu, de **Indeling** wordt weergegeven in het deelvenster. U kunt een specifieke indeling kiezen waarin getallen worden weergegeven aan de gebruiker.
...
Het dialoogvenster Ontwerpen wordt gebruikt om CSS-stijlen voor een component te definiëren en te beheren. De Adaptive Forms Number Input Core Component ondersteunt de AEM [Stijlsysteem](/help/get-started/authoring.md#component-styling).
